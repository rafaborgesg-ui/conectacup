...
const DEFAULT_TIMEOUT_MS = 10000; // 10 segundos de timeout padrão
...
async function apiCall(endpoint: string, options: RequestInit = {}, timeout?: number) {
  ...
  try {
    ...
    // Se for erro de rede ou timeout, retorna um erro mais detalhado
...
export async function fetchStockEntries(): Promise<StockEntry[]> {
  const result = await apiCall('/stock-entries', {}, LONG_TIMEOUT_MS);
  return result.data || [];
}
